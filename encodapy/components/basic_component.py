...
class BasicComponent:
    ...
    def set_component_static_data(self,
        ...
        if isinstance(static_config, IOModell):
            ...
            for static_config_item in static_config.root.keys():
                ...
                static_config_data[static_config_item] = ControllerComponentStaticDataAttribute(
            ...
            static_config_data[static_config.entity] = ControllerComponentStaticDataAttribute(
                value=datapoint_value,
                unit=datapoint_unit
            )
        try:
            static_data = ControllerComponentStaticData.model_validate(static_config_data)
        except ValidationError as error:
            logger.error(f"Error in static data configuration: {error}")

        static_config_available = (len(static_data.root.keys())
                                   == len(self.component_config.staticdata.root.keys()))


        if not static_config_available:
            logger.error("Static data configuration does not match the component configuration. "
                         "Please check the configuration.")
            return
        self.static_data = static_data

    def get_component_static_data(self,
                                  component_id: str,
